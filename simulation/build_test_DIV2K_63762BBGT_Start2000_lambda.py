    ...
    def blur(
        ...
        for y0 in range(0, H, step):
            for x0 in range(0, W, step):
                ...
                cy = (y0 + y1 - 1) * 0.5
                cx = (x0 + x1 - 1) * 0.5
                ker = self.psf.get_psf(H_start+cy, W_start+cx, shift=True, normalize=True)
...
if __name__ == '__main__':
    ...
    # save_root = rf'/user/f.zhang2/data/DIV2K/DIV2K_valid_63762BBGT_32bit_noise0d001_movingStart_sn_scale2'
    # save_root = rf'/user/f.zhang2/data/DIV2K/DIV2K_valid_63762BBGT_32bit_noise0d001_Start2000_sn'
    save_root = rf'/user/f.zhang2/data/DIV2K/DIV2K_valid_63762BBGT_32bit_noise0d001_Start2000_sn_scale2'
    ...
    for filepath in file_list:
        ...
        np.save(os.path.join(target_npy_dir, filename[:-len('.png')] + f'_{H_start}_{W_start}.npy'), image.transpose(2, 0, 1)[np.newaxis, ...])
        np.save(os.path.join(input_npy_dir, filename[:-len('.png')] + f'_{H_start}_{W_start}.npy'), image_blurred_noisy.transpose(2, 0, 1)[np.newaxis, ...])
        np.save(os.path.join(input_noNoise_npy_dir, filename[:-len('.png')] + f'_{H_start}_{W_start}.npy'), image_blurred.transpose(2, 0, 1)[np.newaxis, ...])

        cv2.imwrite(os.path.join(target_dir, filename[:-len('.png')] + f'_{H_start}_{W_start}.png'), image_v)
        cv2.imwrite(os.path.join(input_dir, filename[:-len('.png')] + f'_{H_start}_{W_start}.png'), image_blurred_noisy_v)
        cv2.imwrite(os.path.join(input_noNoise_dir, filename[:-len('.png')] + f'_{H_start}_{W_start}.png'), image_blurred_v)
        kernel_whole_v = np.uint16(kernel_whole * (2 ** 16 - 1))[:, :, ::-1]
        cv2.imwrite(os.path.join(kernel_dir, filename[:-len('.png')] + f'_{H_start}_{W_start}.png'), kernel_whole_v)
                # exit()