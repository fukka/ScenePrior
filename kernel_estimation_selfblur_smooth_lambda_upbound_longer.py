...
if __name__ == '__main__':
    ...
    # HR_dir = r'/user/f.zhang2/data/scene/deadleaves_01_clips_v1/test_patch_OD_fast2stageTest_isolated_sn2_32bit_noise0d001/target_npy'
    ...
    HR_dir = r'/user/f.zhang2/data/scene/deadleaves_01_clips_v1/test_patch_OD_63762BBGT_isolated_sn2_32bit_noise0d001/target_npy'
    LR_dir = r'/user/f.zhang2/data/scene/deadleaves_01_clips_v1/test_patch_OD_63762BBGT_isolated_sn2_32bit_noise0d001/input_noNoise_npy'
    Kernel_dir = r'/user/f.zhang2/data/scene/deadleaves_01_clips_v1/test_patch_OD_63762BBGT_isolated_sn2_32bit_noise0d001/kernel_npy'
    replace_name = r'.npy'
    save_kernel_dir = HR_dir + '_kernel_longer'
    ...
    for HR_idx in tqdm.tqdm(range(len(HR_files) - 1, -1, -1)):
        ...
        filename = os.path.basename(HR_file).replace(replace_name, '.png')

        if os.path.exists(os.path.join(save_kernel_dir, filename + '.npy')):
        ...
        HR = torch.from_numpy(np.float32(np.load(HR_file))).cuda()
        HR_gt = torch.from_numpy(np.float32(np.load(LR_file.replace('input_noNoise_npy', 'target_npy')[:-4] + '.npy'))).cuda()
        LR = torch.from_numpy(np.float32(np.load(LR_file[:-4] + '.npy')[:, :, 12:-12, 12:-12])).cuda()
