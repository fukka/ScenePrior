...
import loss
import networks
from util import post_process_3channel_gcenter, run_zssr, save_final_kernel_1x2x, analytic_kernel_torch, analytic_kernel
...
class KernelGAN:
    ...
    def __init__(self, conf):
        ...
        # Define the GAN
        self.G = networks.GeneratorS1_RGB(conf).cuda()
    ...
    def train_g(self):
        ...
        g_pred = self.G.forward(torch.clone(self.g_input))
        ...
        if self.k2x_analytic:
            ...
            final_k2x = np.stack(
                self.unify_kernel_shape(
                    post_process_3channel_gcenter(self.curr_k_2x, n=self.conf.n_filtering, sf=1)
                )
            )
