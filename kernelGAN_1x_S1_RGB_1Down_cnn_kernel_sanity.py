...
class KernelGAN:
    ...
    def __init__(self, conf):
        ...
        self.conf = conf
        no_model_softmax = False
        # Define the GAN
        self.kernel_estimator = TwoHeadsNetwork(
            K=3, blur_kernel_size=13, no_softmax=no_model_softmax
        ).cuda()
    ...
    def calc_curr_k(self, image=None):
        if image is None:
            image = self.g_input
        self.curr_k_1x = self.kernel_estimator.forward(image)
    ...
    def train_g(self):
        ...
        curr_k_1x = self.kernel_estimator.forward(self.d_input)
        ...
        #     g_pred,
        #     self.d_input[:, :, curr_k_1x.shape[2] // 2:-(curr_k_1x.shape[2] // 2), curr_k_1x.shape[3] // 2:-(curr_k_1x.shape[3] // 2)]
        # )
        ...
        # total_loss_g = loss_img + self.calc_constraints(g_pred)
    ...
    def get_final_image_lr(self, image):
        curr_k_1x = self.kernel_estimator.forward(image)
