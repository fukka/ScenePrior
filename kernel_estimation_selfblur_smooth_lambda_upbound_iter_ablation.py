...
if __name__ == '__main__':
    ...
    idx = 0
    pairs = []
    for HR_idx in range(0, len(HR_files), 100):
        ...
        pairs.append((HR, LR, kernel_gt_np))

    # iter_list = range(200, 10000, 200)
    # iter_list = range(200, 10000, 1000)
    iter_list = range(10000, 100, -2000)
    kernel_psnr_avg_list = []
    for iter in iter_list:
        kernel_estimator.num_iter = iter
        # kernel_estimator.reset()
        kernel_psnr_list = []
        for HR, LR, kernel_gt_np in pairs:
        ...
        kernel_psnr_avg = sum(kernel_psnr_list) / len(kernel_psnr_list)
        print('iter:', iter, 'psnr:', kernel_psnr_avg)
        kernel_psnr_avg_list.append(kernel_psnr_avg)

    iter_ablation = [list(iter_list), kernel_psnr_avg_list]

    import pickle
    with open(r'/user/f.zhang2/data/iter_ablation.pkl', 'wb') as f:
        pickle.dump(iter_ablation, f)