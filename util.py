...
def read_image(path, bitDepth=8):
    """Loads an image"""
    if path.endswith('.png'):
        im = cv2.imread(path, -1) / (2**bitDepth - 1.0)
...
def post_process_kC(k, n, sf=2):
    """Move the kernel to the CPU, eliminate negligible values, and centralize k"""
    if not isinstance(k, np.ndarray):
        k = move2cpu(k)
    # Process each channel independently and store centralized kernels
    centralized_kernels = []
    for c in range(k.shape[0]):
        # Zeroize negligible values
        significant_k = zeroize_negligible_val(k[c], n)
        # Force centralization on the kernel
        centralized_k = kernel_shift(significant_k, sf=sf)
        centralized_kernels.append(centralized_k)
    
    # Find the maximum dimensions
    max_height = max(ck.shape[0] for ck in centralized_kernels)
    max_width = max(ck.shape[1] for ck in centralized_kernels)
    
    # Pad each kernel to match the maximum dimensions
    padded_kernels = []
    for ck in centralized_kernels:
        pad_height = (max_height - ck.shape[0]) // 2
        pad_width = (max_width - ck.shape[1]) // 2
        padded_kernel = np.pad(
            ck,
            (
                (pad_height, max_height - ck.shape[0] - pad_height),
                (pad_width, max_width - ck.shape[1] - pad_width)
            ),
            'constant'
        )
        padded_kernels.append(padded_kernel)
    
    # Stack padded kernels along the channel dimension
    processed_k = np.stack(padded_kernels, axis=0)
    
    return processed_k

def post_process_3channel_gcenter(k, n, sf=2, g_idx=1, zeroizeNegligible=True):
    assert len(k) == 3

    kernel_g = k[g_idx]
    if not isinstance(kernel_g, np.ndarray):
        kernel_g = move2cpu(kernel_g)
    g_center_of_mass = measurements.center_of_mass(kernel_g)
    ...
    for i in range(3):
        ...
        if not isinstance(kernel, np.ndarray):
            kernel = move2cpu(kernel)
        if zeroizeNegligible:
            kernel = zeroize_negligible_val(kernel, n)
        ...
        shift_vec = wanted_center_of_mass - g_center_of_mass
...
def analytic_kernel(k):
    ...
    if k.ndim == 2:
        # Single-channel kernel (kxk)
        return _process_single_channel_kernel(k)
    elif k.ndim == 3:
        # Multi-channel kernel (cxkxk)
        return np.stack([_process_single_channel_kernel(k[c]) for c in range(k.shape[0])], axis=0)
    else:
        raise ValueError("Input kernel must be either 2D (kxk) or 3D (cxkxk)")

def _process_single_channel_kernel(k):
    """Calculate the X4 kernel from the X2 kernel for a single channel"""
    k_size = k.shape[0]
...
def analytic_kernel_torch(k):
    """Calculate the X4 kernel from the X2 kernel (for proof see appendix in paper)"""
    k_size = k.shape[-1]
    # Calculate the big kernels size
    big_k = torch.zeros((3 * k_size - 2, 3 * k_size - 2)).to(k.device)
    # Loop over the small kernel to fill the big one
    for r in range(k_size):
        for c in range(k_size):
            big_k[2 * r:2 * r + k_size, 2 * c:2 * c + k_size] += k[r, c] * k
    ...
    cropped_big_k = big_k[crop:-crop, crop:-crop]
    # Normalize to 1
    cropped_big_k = cropped_big_k / cropped_big_k.sum()
    return cropped_big_k[None, None, ...]


def kernel_shift(kernel, sf, no_pad=False):
...
def save_final_kernel_1x2x(k_1, k_2, conf):
    """saves the final kernel and the analytic kernel to the results folder"""
    sio.savemat(os.path.join(conf.output_dir_path, '%s_kernel_x1.mat' % conf.img_name), {'Kernel': k_1})
...
def save_final_kernel_1x2xC(k_1, k_2, conf):
    """saves the final kernel and the analytic kernel to the results folder"""
    sio.savemat(os.path.join(conf.output_dir_path, '%s_kernel_x1.mat' % conf.img_name), {'Kernel': k_1})
    sio.savemat(os.path.join(conf.output_dir_path, '%s_kernel_x2.mat' % conf.img_name), {'Kernel': k_2})
    if conf.X4:
        k_4 = np.stack([analytic_kernel(k_2[c]) for c in range(k_2.shape[0])], axis=0)
        sio.savemat(os.path.join(conf.output_dir_path, '%s_kernel_x4.mat' % conf.img_name), {'Kernel': k_4})
