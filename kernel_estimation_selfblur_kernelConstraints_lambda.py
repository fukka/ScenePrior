...
class KernelEstimator:
    def __init__(self):
        ...
        self.boundaries_loss_1x = loss.BoundariesLoss(k_size=25).cuda()
        self.centralized_loss_1x = loss.CentralizedLoss(
            k_size=25, scale_factor=1
        ).cuda()
    ...
    def calc_constraints_1c(self, kernel):
        loss_boundaries = (
            self.boundaries_loss_1x.forward(kernel=kernel)
        )

        loss_centralized = (
            self.centralized_loss_1x.forward(kernel=kernel)
        )

        loss_sparse = (
            self.sparse_loss.forward(kernel=kernel)
    ...
    def train_g(self):
        ...
        for step in tqdm.tqdm(range(num_iter)):
            ...
            reprojection_loss = mse(g_pred, self.d_input)
            kernel_loss = self.calc_constraints_1c(out_k[0, 0, :, :]) + self.calc_constraints_1c(out_k[0, 1, :, :]) + self.calc_constraints_1c(out_k[0, 2, :, :])
            total_loss = reprojection_loss + kernel_loss
