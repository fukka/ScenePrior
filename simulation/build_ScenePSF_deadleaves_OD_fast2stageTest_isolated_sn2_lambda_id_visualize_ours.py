...
if __name__ == '__main__':
    ...
    for i in range(50):
        for j in range(80):
            k_h = i * size
            k_w = j * size
            new_location = (k_h, k_w)
            distances = np.linalg.norm(psf_reader.locations - np.array(new_location), axis=1)
    ...
    for i in range(25):
        for j in range(40):
            kernel_path = os.path.join(r"/user/f.zhang2/projects/CVPR2026_deblur/results/SwinIR_Scene_deadleavesv1_01_clip00_scale0.20_noise0d001_Fast2StagePSF_multiLR_GAN_test_fast2stageTest_isolated_sn2_2_size128_noise_100k/visualization/deadleaves_01_test_fast2stageTest_isolated_kernel", map_dict[(i, j)])
            kernel = np.load(kernel_path)

            print(kernel.shape)
            kernel = crop_center(kernel.transpose(2, 0, 1), crop_h, crop_w)
    ...
    cv2.imwrite(rf'./kernel_fast2stage_ours.png', kernel_v_whole)