...
def pupil_size(D, lam, pix, size):
    pixrad = np.deg2rad(pix) / 3600
    deltanu = size * pixrad       # Sampling interval
    rpupil = D / lam * deltanu
...
def center(coefficients,size,rpupil):
    ...
    A[
        size//2-rpupil+1: size//2+rpupil+1,
        size//2-rpupil+1: size//2+rpupil+1
    ]= phase(coefficients, rpupil)
    return A
...
def sample_Zernike_psf(size=25):
    rpupil = 10

    coefficients_base = np.random.rand(8) * random.uniform(1.0, 3.0)
    psf_rgb = []
    for _ in range(3):
        coefficients = coefficients_base + np.random.randn(8) * 0.3
        ...
        psf_c = fftshift(mtf)
        psf_c = crop_center(psf_c, size, size)
        psf_rgb.append(psf_c / psf_c.sum())
