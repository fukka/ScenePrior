...
if __name__ == '__main__':
    ...
    SCENE_LIST = ['deadleaves_window_view']
    HR_DICT = {
        ...
        'deadleaves_window_view': '/user/f.zhang2/data/scene/cross_scene/deadleaves_window_view/IMG_5689_homography_crop.png',
    ...
    for folder in SCENE_LIST:
        ...
        root = os.path.join('/user/f.zhang2/data/scene/cross_scene', folder)
        if not os.path.exists(os.path.join(root, 'homography')):
            os.mkdir(os.path.join(root, 'homography'))
        ...
        for i in range(H // size):
            for j in range(W // size):
                ...
                np.save(rf'{root}/homography/kernel_homography_{i}_{j}.npy', kernel_pred_np)

                kernel_pred_np_v = np.uint16(kernel_pred_np / np.max(kernel_pred_np) * (2 ** 16 - 1))
                cv2.imwrite(rf'{root}/homography/kernel_homography_{i}_{j}.png', kernel_pred_np_v)

                kernel_pred_np_v_whole[i * crop_h:(i+1) * crop_h, j * crop_w:(j+1) * crop_w] = kernel_pred_np_v

        cv2.imwrite(rf'{root}/homography/kernel_homography.png', kernel_pred_np_v_whole)
