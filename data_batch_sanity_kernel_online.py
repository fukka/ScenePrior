...
class DataGenerator(Dataset):
    ...
    def __init__(self, conf, gan, image_clean, image_lr, image_kernel):
        # Default shapes
        ...
        self.shave_edges(scale_factor=1, real_image=conf.real_image)
    ...
    def __getitem__(self, idx):
        ...
        k_in = self.next_crop(for_g=True, for_k=True, idx=idx)
        #
        # return g_in, d_in
        # g_in_list = []
        # d_in_list = []
        ...
        return g_in, d_in, k_in

    def next_crop_single(self, for_g, idx, img_idx, for_k=False):
        ...
        if for_k:
            crop_im = self.image_kernel[img_idx, top:top + size, left:left + size, :]
            return torch.FloatTensor(np.transpose(crop_im, (2, 0, 1))).unsqueeze(0).cuda()
        else:
            if for_g:
    ...
    def next_crop(self, for_g, idx, for_k=False):
        crop_im_list = []
        for img_idx in range(self.image_clean.shape[0]):
            crop_im = self.next_crop_single(for_g, idx, img_idx, for_k=for_k)
    ...
    def shave_edges(self, scale_factor, real_image):
        ...
        if not real_image:
            ...
            self.image_kernel = self.image_kernel[:, 10:-10, 10:-10, :]
        ...
        if sf > 1:
            ...
            self.image_kernel = self.image_kernel[:, :-(shape[1] % sf), :, :] if shape[1] % sf > 0 else self.image_kernel
            self.image_kernel = self.image_kernel[:, :, :-(shape[2] % sf), :] if shape[2] % sf > 0 else self.image_kernel
