...
# Data path
# ============================================================
PATTERN_PATH = "/Users/f.zhang2/Downloads/SuperDeadLeaves.png"
OUT_DIR = "./out_numpy_clips_coverage_"
...
# SCALES = [0.40, 0.45, 0.50, 0.55, 0.60, 0.65, 0.7, 0.75, 0.80, 0.90]
SCALES = [0.40]
...
def sample_clip_shift_around_test_pose(a: Pose, r: float, t: float):
    start_angle_deg = 0

    start_angle = math.radians(start_angle_deg)

    step = 2.0 * math.pi
    theta = start_angle + (t / 10.0) * step
    ...
    return_pose = Pose(
        ...
        ry_deg=a.ry_deg,
        rz_deg=a.rz_deg,
        tx=max(min(a.tx + dx, XY_BOUND), -XY_BOUND),
        ty=max(min(a.ty + dy, XY_BOUND), -XY_BOUND),
        tz=a.tz,
        zoom=a.zoom,
    )
...
def generate_clip_from_test_pose(img0: np.ndarray, test_pose: Pose, spec: TrajectorySpec, out_w: int, out_h: int) -> Dict[str, object]:
    ...
    for t in range(spec.T)[::-1]:
        ...
        p_nom = sample_clip_shift_around_test_pose(test_pose, r=R_MAX*spec.scale, t=t)
...
def main():
    img0 = np.float32(cv2.imread(PATTERN_PATH, -1)[:, :, ::-1]) / 255.0
    if img0.shape[2] < 3:
