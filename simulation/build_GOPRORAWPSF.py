...
def render(image, meta):
  ...
  image_tf = tf.clip_by_value(unprocess.safe_invert_gains(
    image_tf, 1 / meta['rgb_gain'], 1 / meta['red_gain'], 1 / meta['blue_gain']
  ), clip_value_min=0.0, clip_value_max=1.0)
  image_tf = tf.clip_by_value(unprocess.apply_ccm(image_tf, meta['cam2rgb']), clip_value_min=0.0, clip_value_max=1.0)
  image_tf = tf.maximum(image_tf, 1e-8) ** (1 / 2.2)
  image_tf = image_tf * image_tf * (3.0 - 2.0 * image_tf)
  return image_tf


def create_example(image, noise_level=0.2):
  ...
  if noise_level > 0:
      ...
      psf_image_random = np.copy(psf_image)

  # Apply psf
  image_lr = blur_psf(image_np, psf_image_random).clip(0, 1)
  image_lr_tf = tf.convert_to_tensor(image_lr)

  # Render LR RGB
  image_lr_rgb = tf.clip_by_value(render(image_lr, metadata), clip_value_min=0.0, clip_value_max=1.0)
...
if __name__ == '__main__':
  # ori_root = r'/user/f.zhang2/projects/HI-Diff/datasets/GoPro_small/target'
  # save_root = r'/user/f.zhang2/projects/HI-Diff/datasets/GoPro_small/inputPSF'

  ori_root = r'/user/f.zhang2/data/GoPro/train/target'
  save_root = r'/user/f.zhang2/data/GoPro/train/inputPSF'

  # ori_root = r'/user/f.zhang2/data/GoPro/test/GoPro/target'
  # save_root = r'/user/f.zhang2/data/GoPro/test/GoPro/inputPSF'
  ...
  for filepath in file_list:
    ...
    # else:
    #   continue
    if os.path.exists(os.path.join(save_root_lr_rgb, filename)):
        continue
