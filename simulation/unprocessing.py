...
def mosaic(image):
  ...
  mosaiced = np.ones((image.shape[0], image.shape[1], 1))
  mosaiced[0::2, 0::2, 0] = image[0::2, 0::2, 0]
  mosaiced[0::2, 1::2, 0] = image[0::2, 1::2, 1]
  mosaiced[1::2, 0::2, 0] = image[1::2, 0::2, 1]
  mosaiced[1::2, 1::2, 0] = image[0::2, 0::2, 2]
  # mosaiced = mosaiced[:, :, 0:1]
  mosaiced = tf.cast(tf.convert_to_tensor(mosaiced), image.dtype)
  return mosaiced
...
def unprocess(image):
  ...
  with tf.name_scope(None, 'unprocess'):
    ...
    image = tf.clip_by_value(image, 0.0, 1.0)
    # Skip Bayer mosaic before PSF blur
    # image = mosaic(image)
