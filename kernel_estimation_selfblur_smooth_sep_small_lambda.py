...
class net_kernel_RGB(nn.Module):
    ...
    def forward(self):
        ...
        out_k = torch.cat((
            functional.gaussian_blur(img=out_k_R, kernel_size=5, sigma=0.8),
            functional.gaussian_blur(img=out_k_G, kernel_size=5, sigma=0.8),
            functional.gaussian_blur(img=out_k_B, kernel_size=5, sigma=0.8),
        ), dim=1)
        return out_k
...
if __name__ == '__main__':
    ...
    save_kernel_dir = HR_dir + '_kernel_sep_small'
    if not os.path.exists(save_kernel_dir):
        os.makedirs(save_kernel_dir)
    if save_kernel_v:
        save_kernel_v_dir = HR_dir + '_kernel_sep_small_v'
    ...
    for HR_idx in tqdm.tqdm(range(len(HR_files))):
        ...
        if save_kernel_v and kernel_psnr < 40:
