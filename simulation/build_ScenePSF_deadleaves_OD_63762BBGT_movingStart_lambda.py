...
if __name__ == '__main__':
    ...
    # save_root = r'/user/f.zhang2/data/scene/deadleaves_01_clips_v2/test_patch_OD_63762BBGT_isolated_32bit_noise0d001_movingStart_sn_scale2'
    save_root = r'/user/f.zhang2/data/scene/deadleaves_01_clips_v2/test_patch_OD_63762BBGT_isolated_32bit_noise0d001_movingStart_sn'
    ...
    for filepath in file_list[:1]:
        ...
        H_range, W_range = max(H_sensor - patch_size, 0), max(W_sensor - patch_size, 0)
        H_step = 200
        W_step = 200
        ...
        for H_start in range(0, H_range, H_step):
            for W_start in range(0, W_range, W_step):
                ...
                kernel = psf_obj.get_psf(H_start+y, W_start+x, shift=True, normalize=True).transpose((2, 0, 1))
                ...
                np.save(os.path.join(target_npy_dir, filename[:-len('.png')] + f'_{H_start+y}_{W_start+x}.npy'), sharp_patch)
                np.save(os.path.join(input_npy_dir, filename[:-len('.png')] + f'_{H_start+y}_{W_start+x}.npy'), blur_noisy_patch)
                np.save(os.path.join(input_noNoise_npy_dir, filename[:-len('.png')] + f'_{H_start+y}_{W_start+x}.npy'), blur_patch)
                np.save(os.path.join(kernel_npy_dir, filename[:-len('.png')] + f'_{H_start+y}_{W_start+x}.npy'), kernel.numpy())
                ...
                cv2.imwrite(os.path.join(target_dir, filename[:-len('.png')] + f'_{H_start+y}_{W_start+x}.png'), sharp_patch_save)
                cv2.imwrite(os.path.join(input_noNoise_dir, filename[:-len('.png')] + f'_{H_start+y}_{W_start+x}.png'), blur_patch_save)
                cv2.imwrite(os.path.join(input_dir, filename[:-len('.png')] + f'_{H_start+y}_{W_start+x}.png'), blur_noisy_patch_save)
                cv2.imwrite(os.path.join(kernel_dir, filename[:-len('.png')] + f'_{H_start+y}_{W_start+x}.png'), kernel_save)
