...
def main():
    ...
    kernels = glob.glob(os.path.join(args.kernel_dir, '*', '*', '*_x1.mat'))[::-1]
    print(f'found {len(kernels)} kernels')
    columns = 4
    rows = math.ceil(len(kernels) / columns)
    if args.visualize_together:
        fig = plt.figure(figsize=(8, rows))
    else:
        # If visualize each separately, must have save_to path
    ...
    for kernel_path in kernels:
        if not args.visualize_together:
            save_to_path = os.path.join(args.save_dir, os.path.basename(kernel_path)[:-len(".mat")]+".png")
            if os.path.exists(save_to_path):
                print(f'skipping {save_to_path}')
                continue
    ...
    if args.visualize_together:
        if args.save_dir:
            plt.savefig(os.path.join(args.save_dir, 'all.png'))
        else:
            plt.show()
    print(f'generated {counter} kernels')
