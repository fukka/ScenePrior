...
class wavefront2psf(nn.Module):
    ...
    def forward(self, WF):
        ...
        phase = torch.exp(-1j * 2 * torch.pi * W)
        # phase = torch.where(phase == 1, 0, phase)
        phase = torch.where(phase == 1, torch.zeros_like(phase).float(), phase.float())
        # clq
        # print(phase.shape)
        phase = fft2(phase)
...
def main():
    ...
    # H = torch.randn(1) * 10
    # H = torch.ones(1)
    # H1 = H.float().repeat(1, 6)
    # H2 = H.float().repeat(1, 3)
    BS = 1
    # coe = torch.randn(1, 9)
    coe_save = np.load('./coe_R.npy')

    for _ in range(5):
        idx = random.randint(0, coe_save.shape[0] - 1)
        coe = torch.from_numpy(coe_save[idx:idx+1, :])

        color = 0
        wavefront = fc11(coe, IS, color, BS)
        psf = fc12(wavefront)
