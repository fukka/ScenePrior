...
class Config:
    def __init__(self):
        ...
        self.parser.add_argument('--override_output', action='store_true', help='override output')

        # Input
        self.parser.add_argument('--input_bitdepth', type=int, default=8, help='input bitdepth')
        ...
        self.parser.add_argument('--mode', type=str, default="original", help='training mode')
        ...
        self.parser.add_argument('--G_structure', type=list, default=[7, 5, 3, 1, 1, 1], help='The kernel size G is estimating')
    ...
    def parse(self, args=None):
        ...
        tmp = []
        for x in self.conf.G_structure:
            if x != " ":
                tmp.append(int(x))
        self.conf.G_structure = tmp
        return self.conf
    ...
    def clean_file_name(self):
        ...
        self.conf.img_name = self.conf.input_image_path.split('/')[-1].replace('ZSSR', '') \
            .replace('real', '').replace('__', '').split('_.')[0][:-len('.npy')]
    ...
    def set_output_directory(self):
        ...
        self.conf.output_dir_path = os.path.join(self.conf.output_dir_path, self.conf.img_name, self.conf.mode)
