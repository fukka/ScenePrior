...
if __name__ == '__main__':
    ...
    save_root = r'/user/f.zhang2/data/scene/deadleaves_01_clips_v2/test_patch_OD_fast2stageTest_32bit_noise0d001'
    from simulation.utils_psf import PSF

    reg_noise_std = 0.001
    kernel_size = 25
    patch_size = 128
    kernel_sample_ratio = 0.5
    ...
    for filepath in file_list:
        ...
        while not covered_all:
            ...
            senter_H_start = iteration * (H * kernel_sample_ratio)
            senter_W_start = iteration * (W * kernel_sample_ratio)
            if senter_H_start > H_sensor and senter_W_start > W_sensor:
                covered_all = True
                senter_H_start = H_sensor - (H * kernel_sample_ratio) + 1
                senter_W_start = W_sensor
            elif senter_H_start > H_sensor:
                senter_H_start = H_sensor - (W * kernel_sample_ratio) + 1
            elif senter_W_start > W_sensor:
                senter_W_start = W_sensor - W + 1
            ...
            for y in y_starts:
                ...
                for x in x_starts:
                    ...
                    _, _, kernel = psf_reader.get_psf_by_location(new_location=(center_patch_y_sensor * kernel_sample_ratio, center_patch_x_sensor * kernel_sample_ratio))
                    ...
                    np.save(os.path.join(target_npy_dir, filename[:-len('.png')] + f'_{iteration}_{y}_{x}.npy'), sharp_patch)
                    np.save(os.path.join(input_npy_dir, filename[:-len('.png')] + f'_{iteration}_{y}_{x}.npy'), blur_noisy_patch)
                    np.save(os.path.join(input_noNoise_npy_dir, filename[:-len('.png')] + f'_{iteration}_{y}_{x}.npy'), blur_patch)
                    np.save(os.path.join(kernel_npy_dir, filename[:-len('.png')] + f'_{iteration}_{y}_{x}.npy'), kernel.numpy())
                    ...
                    cv2.imwrite(os.path.join(target_dir, filename[:-len('.png')] + f'_{iteration}_{y}_{x}.png'), sharp_patch_save)
                    cv2.imwrite(os.path.join(input_noNoise_dir, filename[:-len('.png')] + f'_{iteration}_{y}_{x}.png'), blur_patch_save)
                    cv2.imwrite(os.path.join(input_dir, filename[:-len('.png')] + f'_{iteration}_{y}_{x}.png'), blur_noisy_patch_save)
            ...
            cv2.imwrite(os.path.join(save_root, filename[:-len('.png')] + f'_input_{iteration}.png'), blur_noisy_image)
            cv2.imwrite(os.path.join(save_root, filename[:-len('.png')] + f'_input_noNoise_{iteration}.png'), blur_image)
            cv2.imwrite(os.path.join(save_root, filename[:-len('.png')] + f'_target_{iteration}.png'), sharp_image)
            cv2.imwrite(os.path.join(save_root, filename[:-len('.png')] + f'_kernel_{iteration}.png'), kernel_image)
