...
if __name__ == '__main__':
    ...
    visualize = True
    # visualize_root = r'/group-volume/Fengjia-Contents/data/kernelGAN_data/evaluation_results/PatternRAWPSF_gcenter_J_MKPD_raw_5x5demosaic'
    visualize_root = r'/group-volume/Fengjia-Contents/data/kernelGAN_data/evaluation_results/PatternRAWPSF_gcenter_J_MKPD_raw_3x3demosaic'
    if visualize:
    ...
    for img_idx, val_data in enumerate(dataset):
        ...
        # TODO img_RAW
        # demosaicer = Debayer5x5().cuda()
        demosaicer = Debayer3x3().cuda()
        img_RAW_demosaiced = demosaicer(torch.from_numpy(img_RAW).permute((2, 0, 1)).unsqueeze(0).float().cuda())
        img_RAW_demosaiced = img_RAW_demosaiced[0].permute((1, 2, 0)).cpu().numpy()
        psf_pred = inference.estimate_kernel(img_RAW_demosaiced)
