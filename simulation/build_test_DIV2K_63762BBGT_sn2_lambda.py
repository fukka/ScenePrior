...
if __name__ == '__main__':
    ...
    save_root = r'/user/f.zhang2/data/DIV2K/DIV2K_valid_63762BBGT_32bit_noise0d001_sn2'
    ...
    senter_H_start, senter_W_start = 0, 0
    ...
    for filepath in file_list:
        ...
        blur_image_np = np.zeros((3, stride * len(y_starts), stride * len(x_starts)))
        blur_noisy_image_np = np.zeros((3, stride * len(y_starts), stride * len(x_starts)))
        sharp_image_np = np.zeros((3, stride * len(y_starts), stride * len(x_starts)))

        blur_image = np.zeros((stride * len(y_starts), stride * len(x_starts), 3), dtype=np.uint16)
        blur_noisy_image = np.zeros((stride * len(y_starts), stride * len(x_starts), 3), dtype=np.uint16)
        sharp_image = np.zeros((stride * len(y_starts), stride * len(x_starts), 3), dtype=np.uint16)
        ...
        for y in y_starts:
            ...
            for x in x_starts:
                ...
                kernel = psf_obj.get_psf(center_patch_y_sensor, center_patch_x_sensor)
                ...
                blur_image_np[:, y0: y0 + patch_size, x0: x0 + patch_size] = blur_patch.numpy()
                blur_noisy_image_np[:, y0: y0 + patch_size, x0: x0 + patch_size] = blur_noisy_patch.numpy()
                sharp_image_np[:, y0: y0 + patch_size, x0: x0 + patch_size] = sharp_patch.numpy()
                ...
                blur_image[y0: y0 + patch_size, x0: x0 + patch_size] = blur_patch_save
                blur_noisy_image[y0: y0 + patch_size, x0: x0 + patch_size] = blur_noisy_patch_save
                sharp_image[y0: y0 + patch_size, x0: x0 + patch_size] = sharp_patch_save
                kernel_image[
                    y_idx * kernel_size: (y_idx + 1) * kernel_size, x_idx * kernel_size: (x_idx + 1) * kernel_size
                ] = kernel_save

        np.save(os.path.join(input_npy_dir, filename[:-len('.png')] + f'.npy'), np.float32(blur_noisy_image_np)[np.newaxis, ...])
        np.save(os.path.join(input_noNoise_npy_dir, filename[:-len('.png')] + f'.npy'), np.float32(blur_image_np)[np.newaxis, ...])
        np.save(os.path.join(target_npy_dir, filename[:-len('.png')] + f'.npy'), np.float32(sharp_image_np)[np.newaxis, ...])

        cv2.imwrite(os.path.join(input_dir, filename[:-len('.png')] + f'.png'), blur_noisy_image)
        cv2.imwrite(os.path.join(input_noNoise_dir, filename[:-len('.png')] + f'.png'), blur_image)
        cv2.imwrite(os.path.join(target_dir, filename[:-len('.png')] + f'.png'), sharp_image)