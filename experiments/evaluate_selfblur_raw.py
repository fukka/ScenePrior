...
if __name__ == '__main__':
    ...
    visualize = True
    visualize_root = r'/group-volume/Fengjia-Contents/data/kernelGAN_data/PatternRAWPSF_gcenter_selfblur_raw_whole'
    if visualize:
    ...
    for img_idx, val_data_list in enumerate(dataset):
        ...
        img_RAW = np.stack([val_data['img_RAW'] for val_data in val_data_list], axis=0)
        ...
        # psf_pred = inference.estimate_kernel_raw(img_RAW)
        psf_pred, img_HR_pred = inference.estimate_kernel_raw(img_RAW, return_image_HR=True)
        if not RGB:
